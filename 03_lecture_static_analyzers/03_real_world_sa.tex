...
\begin{tikzpicture}[remember picture]
   ...
   \onslide<6->{
      \node[xshift=42.5mm,yshift=-7mm] (llvm) at (current bounding box.center) {\raisebox{-.25\height}{\Logo[1.25cm]{https://llvm.org}{llvm}}~\textsb{Clang}};
   ...
   \coordinate (ul) at (astree.north west);
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<8-|handout:2>{
      \fill[white,fill opacity=.85] ([xshift=5mm]ll) -| (ur) -| (ul) -| cycle;
      \node[text width=7.5cm] at (current page.center) {%
      \begin{multicols}{2}         
            \begin{itemize}
               \itemsep8pt
               \item<9->  \strut\smash{\hyperref[sonarlint]{SonarLint}}
               \item<10-> \strut\smash{\hyperref[astree]{Astrée} \onslide<11->{\footnotesize(sound!)}}
               \item<12-> \strut\smash{\hyperref[lisa]{LISA}}
               \item<13-> \strut\smash{\hyperref[java-language-server]{Java \rlap{Language Server}}}
               \item<14-> \strut\smash{\hyperref[lintr]{lintr}}
               \item<15-> \strut\smash{\hyperref[flowr]{flowR}}
...
\subsection{SonarLint}\label{sonarlint}
\begin{frame}{SonarLint}
\begin{itemize}
   \item Support for multiple languages (15+) \begin{itemize}
      \item Widely varying support and rules
      \item We focus on \href{https://github.com/SonarSource/sonar-java}{\textsb{sonar-java}}
   \end{itemize}
\end{itemize}
\begin{tikzpicture}[overlay,remember picture]
   \node[below left=4mm] at(current page.north east) {\Logo[2.5cm]{https://www.sonarsource.com/products/sonarlint/}{sonar}};
   \node[above right,yshift=5mm,bottom note] at(current page.south west) {\url{https://github.com/SonarSource}};
\end{tikzpicture}
\end{frame}

\subsection{Astrée}\label{astree}
\begin{frame}{Astrée}
   Hey!
\end{frame}

\subsection{LISA}\label{lisa}
\begin{frame}{LISA}
   Hey!
\end{frame}

\subsection{Java Language Server}\label{java-language-server}
\begin{frame}{Java Language Server}
   Hey!
\end{frame}

\subsection{lintr}\label{lintr}
\begin{frame}{lintr}
   Hey!
\end{frame}

\subsection{flowR}\label{flowr}
\begin{frame}{flowR}
   Hey!
\end{frame}
