...
\begin{tikzpicture}[overlay,remember picture]
   \onslide<2->{\node[above left,yshift=4mm,inner sep=0pt] (@) at(current page.south east) {\includegraphics[height=4.5cm]{sieglinde.png}};
   \node[above left,yshift=3mm] at (@.south west) {\fontseries{sb}\selectfont\tiny Sieglinde, vollzeit-cutie, hoch-motiviert};}
   \onslide<3->{
      \node[above right,yshift=3.25mm,inner sep=0pt] (@2) at(current page.south west) {\includegraphics[width=3cm]{hans-houdini.png}};
      \node[above right,xshift=-7.5mm,yshift=3mm] at (@2.south east) {\fontseries{sb}\selectfont\tiny Hans Houdini, mag Kraulen mehr als Abstract Interpretation};
   }
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<5->{
      \tikzset{@/.style={}}
      \only<6->{
         \tikzset{@/.style={lightgray}}
      }
      \node[below,align=center,@,font=\large,yshift=15mm] (@) at(current page.center|-@.south) {A theory and framework to \textit{systematically} derive sound\\static analyses by \textsb{abstracting} program semantics.\rlap{\supercite{cousout2021principles}}
      };
      \onslide<7->{
         \node[align=center,below,font=\large] at (@.south) {%
            A way to soundly over-approximate all possible program behaviors.
         };
      }
   }
\end{tikzpicture}
...
\begin{itemize}
   ...
   \begin{tikzpicture}[overlay,remember picture]
      ...
      \onslide<6->{
         \draw[Kite-,gray] ([yshift=1mm]sonar-java.south east) to[out=-30,in=180] ++(5mm,-1.33mm) node[right,bottom note] {733 rules!};
      }
   \end{tikzpicture}
\end{itemize}
\noindent
...
\onslide<23->{
   ...
   \begin{align*}
      ...
      \end{cases}
   \end{align*}
   With fixpoint iteration% gen: variables used in n without being re-defined first; kill: variables assigned a value in n
   };
   \endpgfinterruptboundingbox
...
\begin{itemize}
   \itemsep9pt
   \item<2-> Analyze \enquote{Dead Stores} {\onslide<3->{\smaller[2]\textcolor{gray}{(in 312\,loc\,/\,260\,cloc)}:}}
\begin{minted}[escapeinside=||]{java}
|\onslide<4->|int x = 0;
x = 42;|\onslide<1->|
\end{minted}
   \item<5-> Use \link{https://github.com/SonarSource/sonar-java/blob/c9d7e50b397abd1b67fbcb2a8f1648de64fc4ac9/java-checks/src/main/java/org/sonar/java/checks/DeadStoreCheck.java\#L77}{liveness analysis} to obtain \(out_n\) of each basic block in the CFG
...
\begin{tikzpicture}[overlay,remember picture]
   \node[below left=4mm] at(current page.north east) {\Logo[2.5cm]{https://www.sonarsource.com/products/sonarlint/}{sonar}};
   \node[above right,bottom note, yshift=4mm] at(current page.south west) {\link{https://github.com/SonarSource/sonar-java/blob/c9d7e50b397abd1b67fbcb2a8f1648de64fc4ac9/java-checks/src/main/java/org/sonar/java/checks/DeadStoreCheck.java\#L52}{Implementation of Rule S1854}};
...
\onslide<15->{
   \node[below=1mm,align=right,align=center,font=\footnotesize] (a-domains) at(theory.south) {\small A lot of\ldots\\\link{https://github.com/eclipse-jdtls/eclipse.jdt.ls/blob/82bdb0d12293fff42518bb72b6c56a05c2a85e94/org.eclipse.jdt.ls.core/src/org/eclipse/jdt/ls/core/internal/corrections/RefactorProcessor.java}{AST traversal}}; %
...
\begin{tikzpicture}
   ...
   \only<2->{\tikzset{@base/.style={opacity=.125}}}
   ...
   \only<7|handout:3>{\node[@infer,left=1.5mm,align=left] at(llvm.west) {Determine registers, scheduling, types~\ldots\\
   For example:\\[3pt]\strut~~~~~~~~~\(\displaystyle\frac{
         \mathcolor{gray}{\Gamma \vdash{}} e_1 : \texttt{int} \quad \mathcolor{gray}{\Gamma \vdash{}} e_2 : \texttt{int}
      }{
         \mathcolor{gray}{\Gamma \vdash{}} e_1 + e_2 : \texttt{int}
      }\)
   };}
