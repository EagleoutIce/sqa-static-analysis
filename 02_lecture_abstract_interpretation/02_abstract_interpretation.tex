...
\begin{onlyenv}<7-|handout:2->
   ...
   \onslide<9->{%
   \draw[history-event] (3.35,1) -- ++(.25,1.45) -- ++(.25,0) \historybox{right}{1986}{Foundations}{\citeauthor*{DBLP:journals/toplas/ClarkeES86}~\cite{DBLP:journals/toplas/ClarkeES86}};
   }
   \onslide<10->{%
   \draw[history-event] (4.5,1) -- ++(.25,.65) -- ++(.25,0) \historybox{right}{2004}{Bounded MC}{\citeauthor*{clarke2004tool}~\cite{clarke2004tool}};
   }
\end{onlyenv}
   \onslide<1->
\end{tikzpicture}
