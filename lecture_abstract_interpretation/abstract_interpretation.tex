...
\tikzset{ImageScales/.style={xscale=1.2,yscale=1}}
...
\begin{onlyenv}<7-|handout:2->
   ...
   \draw[history-event,@] (4,3) -- ++(.25,1.45) -- ++(.25,0) \historybox{right,@}{1992}{Theorem Prover}{PVS, \citeauthor*{DBLP:conf/cade/OwreRS92}\;\supercite{DBLP:conf/cade/OwreRS92}};
   \onslide<8->{%
   \draw[history-event,@] (4.4,3) -- ++(.25,.75) -- ++(.25,0) \historybox{right,@}{2004}{Proof Asisstant}{Coq, \citeauthor*{DBLP:series/txtcs/BertotC04}\;\supercite{DBLP:series/txtcs/BertotC04}}; % isabelle, agda, ...
   }
   \tikzset{@/.style={}}\only<11-|handout:0>{\tikzset{@/.style={lightgray}}}
   % DBLP:journals/toplas/ClarkeES86
   \onslide<9->{%
   \draw[history-event,@] (3.5,1) -- ++(.25,1.45) -- ++(.25,0) \historybox{right,@}{1986}{Foundations}{\citeauthor*{DBLP:journals/toplas/ClarkeES86}\;\supercite{DBLP:journals/toplas/ClarkeES86}};
   }
   \onslide<10->{%
   \draw[history-event,@] (4.4,1) -- ++(.25,.75) -- ++(.25,0) \historybox{right,@}{2004}{Bounded MC}{\citeauthor*{clarke2004tool}\;\supercite{clarke2004tool}};
   }
   \tikzset{@/.style={}}\only<13-|handout:0>{\tikzset{@/.style={lightgray}}}
   \onslide<11->{%
   \draw[history-event,@] (3.15,-1) -- ++(.25,1.45) -- ++(.25,0) \historybox{right,@}{1974\,\&\,75}{Foundations}{\citeauthor*{DBLP:conf/relsoft/BoyerEL75}\;\supercite{DBLP:conf/relsoft/BoyerEL75}, \citeauthor*{DBLP:conf/ibm/King74}\;\supercite{DBLP:conf/ibm/King74}};
   }
   \onslide<12->{%
   \draw[history-event,@] (4.5,-1) -- ++(.25,.75) -- ++(.25,0) \historybox{right,@}{2008}{Automation}{KLEE, \citeauthor*{DBLP:conf/osdi/CadarDE08}\;\supercite{DBLP:conf/osdi/CadarDE08}};
   }
   \onslide<13->{%
      \draw[history-event] (3.2,-3) -- ++(.25,1.45) -- ++(.25,0) \historybox{right}{1977}{Fixpoints on Lattices}{\citeauthor{DBLP:conf/popl/CousotC77}\;\supercite{DBLP:conf/popl/CousotC77}};
   }
   
   \onslide<14->{%
      \draw[history-event] (4.4,-3) -- ++(.25,.75) -- ++(.25,0) \historybox{right}{2004}{Automated Application}{\citeauthor{DBLP:conf/ifip/Mauborgne04}\;\supercite{DBLP:conf/ifip/Mauborgne04}};
   }
\end{onlyenv}
   \onslide<1->
\end{tikzpicture}
...
\strut\hfill\raisebox{\dimexpr-\height+1cm}{\begin{tikzpicture}
   \onslide<2->{%
      ...
      \node[above] at(current bounding box.north) {\clap{\small\strut Using Analyses\;\supercite{rival2020introduction}}};
   }
\end{tikzpicture}}\hfill\raisebox{\dimexpr-\height+1cm}{\begin{tikzpicture}
   \onslide<3->{%
      ...
      \node[above] at(current bounding box.north) {\clap{\small\strut Formal Foundations\;\supercite{cousout2021principles}}};
   }
\end{tikzpicture}}\hfill\raisebox{\dimexpr-\height+1cm}{\begin{tikzpicture}
   \onslide<4->{%
      \draw[darkgray,thick,rounded corners=2pt,path image={\includegraphics[width=4cm]{dfa-cover.jpg}}] (0,0) rectangle ++(3.9,6);
      \node[above] at(current bounding box.north) {\clap{\small\strut Dataflow Perspective\;\supercite{10.5555/1592955}}};
   }
...
\end{lrbox}
\begin{tikzpicture}[overlay,remember picture]
   \node[xshift=2.5mm,yshift=-9mm] at(current page.center) {\usebox\GraphHeaven};
...
\endpgfonlayer
\onslide<7->{%
   \node[below=1mm] at(current bounding box.south) {\clap{\scriptsize Intervals \absexpr{x \in \IntCC ab} TODO: y}};
}
...
\end{tikzpicture}\bigskip
   \item<8-> But, how do we know that \bjava{int a = 0;} implies \AbstractInfo{a = 0}?
   \begin{itemize}
      \itemsep3.5pt
...
\begin{frame}{Galois Connection --- Linking Posets\quad\textcolor{gray}{\footnotesize(small detour)}}
   TODO: Galois
\end{frame}

\begin{frame}
   TODO: But how to handle control flow? Loops? Recursion?
\end{frame}
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<14-|handout:3>{
      \node[right=5.15cm] (int@i1@sign) at (int@i1.east) {\AbstractInfo{x_0 = 0}};
...
\begin{frame}
   TODO: other domains, abstractions, show pentagon, lisa live show
   
   TODO: small and bigstep semantics!!
   
   TODO: Use intervals, show now we dont knwo because stupid widen, refine with octagon, tradeoff speed, precision etc.
...
\begin{frame}
   TODO: answer some questions from last lecture, add new
   TODO: add figureheads of people
\end{frame}
