...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \only<6-8|handout:1>{%
      \draw[Kite-,line cap=round,gray] ([yshift=2.95mm,xshift=1.85mm]pic cs:intersect-on-cond) to[out=75,in=180] ++(5mm,3mm) node[right,font=\footnotesize] {\tiny if we are inside the loop we know that \textit{x < 2} holds!};
   }%
   \onslide<7->{%
