...
\begin{tikzpicture}[Rect/.append style={font=\scriptsize,align=left,inner sep=3.5pt},remember picture]
   ...
   \draw[gray,Link] (int@i3.south) |- (int@i4.east);
...
\begin{tikzpicture}[Rect/.append style={font=\scriptsize,align=left,inner sep=3.5pt},remember picture]
   ...
   \draw[gray,Link] (long@i3.south) |- (long@i4.east);
...
\begin{frame}{Let's Bring it All Together}
\begin{tikzpicture}[overlay,remember picture]
   ...
   {\begin{enumerate}
      ...
      \item<3-> Define the lattice! \textcolor{gray}{\footnotesize(there are many solutions)}\hfill{
         \llap{\smash{%
\raisebox{-\height+\baselineskip}{\onslide<7-|handout:2>{%
\begingroup
\scriptsize
\begin{tikzpicture}[line cap=round,x=7.5mm,y=3mm,gray]
...
\endgroup
      ...
      \item<4-> Define the abstract semantics on the following language!\\[3pt]
\begin{bnf}
\(\expr\) ::= \(c\) : \Comment{\text{constant}, \subnode{constant}{c \in \R}\vspace*{\baselineskip}}
   | \(\expr_1 + \expr_2\) : \Comment{\text{\subnode{add}{addition}}\vspace*{\baselineskip}}
   ;;
\end{bnf}\begin{tikzpicture}[overlay,remember picture]
   \onslide<8-|handout:2>{%
      \node[right=20mm,yshift=.25mm,text width=8cm,font=\scriptsize,gray] (@1) at (constant.east) {%
      ...
      \node[below right,yshift=.25mm+.6\baselineskip,text width=8cm,font=\scriptsize,gray] at (add.east-|@1.west) {%
         \absexpr{\EvaluateExpr{\texttt{a + b}}\Environment \asdef\eq 
         \EvaluateExpr{\texttt{a}}\Environment \oplus \EvaluateExpr{\texttt{b}}\Environment
         }\\[4pt]
         \absexpr{\bot/\top/0 \oplus x = \bot/\top/0},\quad
         ~~\absexpr{x \oplus y = y \oplus x}\\[3pt]
         ~~~~~~~\absexpr{x \oplus x = x},\quad \absexpr{\geq\!0 {}\oplus{} \!\leq\!0 = \top}
      };%
   }
\end{tikzpicture}
      \item<5-> Provide a concretization for~\absexpr{\gamma(\text{0})} and 
      \absexpr{\gamma(\leq\!0)}!\\[3pt]
      \onslide<9-|handout:2>{%
         \textcolor{gray}{%
      ...
      \item<6-> Do we need widening? If so, define \absexpr{\widen}!\\[3pt]
         \onslide<10-|handout:2>{\textcolor{gray}{\textit{We do not need widening here as our lattice is finite and our semantics do not introduce new elements. (We will concretize this next week.)}}}
   \end{enumerate}}
   };
   }
\end{tikzpicture}  
...
\begin{frame}
   ...
   TODO: Relational domain, product domain
...
\begin{frame}{Summarize}
   TODO: summarize important parts, show with 
\end{frame}
