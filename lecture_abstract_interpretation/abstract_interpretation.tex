...
\begin{tikzpicture}[line cap=round]
   ...
   \onslide<10->{
      \node[below right,xshift=-2mm,yshift=5mm,font=\footnotesize,text width=6cm,opacity=.5] at (@doc.south west) {\begin{itemize}
         ...
         \item Each trace is a single execution (test,~\ldots)
   ...
   \only<12->{
   \fill[red,opacity=.175,even odd rule,name path=red-overapprox] plot [smooth] coordinates {(0,0) (1,0.4) (2,0.5) (3,1) (4,.8) (5,1) (6,.1) (7,0.2) (8.03,.2) (8.03,3) (7,2.8) (6,3) (5,2.95) (4,2.85) (3,2.75) (2,2.65) (1,2.5) (0,2) } -- cycle (6,1.85) circle[radius=4mm]; 
   }
   \onslide<11->{%
      \draw[Circle-,red,rounded corners=4pt] (7.75,2.9) -- ++(.35,.5) -- ++(.5,0) node[below right,yshift=.7\baselineskip,align=left] {(Trace) Abstraction\textsuperscript{\cite[92]{cousout2021principles}}\\[-2pt]\footnotesize\color{gray}just one of many\\[-2pt]\footnotesize\color{gray}this \textbf{must} include all concrete traces!\\[-2pt]\footnotesize\color{gray}(over-approximate)};
   }
   \endpgfinterruptboundingbox
   \only<13->{
      \pgfonlayer{background}
      \path[name path=intersector] (4,0) -- ++(0,3);
      \path[name intersections={of=red-overapprox and intersector,by={i1,i2}}];
      \draw[|-|,red,thick,line cap=round,line join=round] ([yshift=-.5pt]i1) -- ([yshift=.5pt]i2) node[above=.5pt,gray,font=\footnotesize] {e.g. \absexpr{x \in \IntCC13}};
      \endpgfonlayer
   }
   \onslide<14->{
...
\newsavebox\HelmutHasse
\savebox\HelmutHasse{%
\ImageWithRoundedCorners{17.25mm}{helmut-hasse.jpg}%
}
\newsavebox\HasseWip
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<4|handout:1>{
      \node[below left=4.65mm,yshift=3mm,align=right,font=\tiny,
         href node={https://opc.mfo.de/detail?photoID=1570}
      ] (@bk) at(current page.north east) {%
         \usebox\HelmutHasse\\[1.65pt]
         \textsb{Helmut Hasse (1898--1979)}\\%
...
\begin{frame}{\insertsubsection}
   \begin{itemize}
      ...
      \item<14-> You may know fixpoints from tools like \LaTeX{} or the \(\lambda\)-calculus
   \end{itemize}
\begin{tikzpicture}[overlay,remember picture]
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<5->{%
      \node[above right,yshift=-1.5pt] (int@i2a) at (int@i2-|int@i1.west) {\AbstractInfo{\only<8-9,12|handout:2,3>{\boldmath}\text{\textcolor{gray}{\scriptsize[pre]}~}x_1 \in \only<-8|handout:1>{\IntCC00}\only<9-11|handout:2>{\IntCC01\quad (\IntCC00 \subnode{union-on-loops}{{}\cup{}} \IntCC11)}\only<12-|handout:3->{\IntCC02\quad (\IntCC01 \cup \IntCC12)}}};
   }%
   \only<9-11|handout:2>{%
      \draw[Kite-,line cap=round,gray] ([yshift=2.95mm,xshift=1.85mm]pic cs:union-on-loops) to[out=75,in=180] ++(5mm,3mm) node[right,font=\footnotesize] {\tiny we join on loops because, at this point \textit{x} can have any of the values};
   }%
   \onslide<6->{%
      \node[below right,yshift=1.5pt] (int@i3a) at (int@i2-|int@i1.west) {\AbstractInfo{\only<10|handout:0>{\boldmath}\text{\textcolor{gray}{\scriptsize[in]}~}x_2 \in \only<-9|handout:1>{\IntCC00\quad (\IntCC00 \subnode{intersect-on-cond}{{}\cap{}} \IntOC{-\infty}1)}\only<10-|handout:2->{\IntCC01\quad (\IntCC01 \cap \IntOC{-\infty}1)}}};
   }%
   \only<6-8|handout:1>{%
      \draw[Kite-,line cap=round,gray] ([yshift=2.95mm,xshift=1.85mm]pic cs:intersect-on-cond) to[out=75,in=180] ++(5mm,3mm) node[right,font=\footnotesize] {\tiny if we are inside the loop we know that \absexpr{x < 2} holds!};
   }%
   \onslide<7->{%
      \node[right] (int@i3) at (int@i3-|int@i1.west) {\AbstractInfo{\only<11|handout:0>{\boldmath}\text{}x_3 \in \only<-10|handout:1>{\IntCC11\quad (\IntCC00 \subnode{interval-plus}{{}\oplus{}} \IntCC11)}\only<11-|handout:2->{\IntCC12\quad (\IntCC01 \oplus \IntCC11)}}};
   }
   \only<7-8|handout:1>{%
      \draw[Kite-,line cap=round,gray] ([yshift=-.95mm,xshift=1.85mm]pic cs:interval-plus) to[out=285,in=180] ++(5mm,-3mm) node[right,font=\footnotesize] {\tiny we have to know how to add intervals here};
   }%
   \onslide<13-|handout:3>{%
      \node[right] (int@i4) at (int@i4-|int@i1.west) {\AbstractInfo{\text{\textcolor{gray}{\scriptsize[post]}~}x_4 \in \IntCC22 \quad (\IntCC02 \subnode{intersect-after-loop}{{}\cap{}} \IntCO2{\infty})}};
   }
   \only<13-|handout:3>{%
      \draw[Kite-,line cap=round,gray] ([yshift=-.95mm,xshift=1.85mm]pic cs:intersect-after-loop) to[out=265,in=0] ++(-5mm,-3mm) node[left,font=\footnotesize] {\tiny after the loop we know that \absexpr{\neg (x < 2) \eq x \geq 2} holds!};
   }%
...
\begin{frame}{Denotational Semantics\qquad \textcolor{gray}{while loops}}
   ...
   \begin{tikzpicture}[overlay,remember picture,line cap=round]
      ...
      \onslide<4->{%
         \draw[-Kite,line cap=round] (while-loop-node.south east) to[out=-30,in=30,looseness=6] node[pos=.5,right,align=left] (@iter) {%
      ...
      \onslide<5->{
         \draw[-Kite,line cap=round,gray] ([xshift=3.5mm,yshift=.5mm]@iter.south west) to[out=-90,in=180] ++(.5,-.15) node[right,font=\scriptsize] {Try to map this to the \textit{Interval Analysis, I} example!};
      }
      \onslide<6->{%
...
\begin{itemize}
   ...
   \item<14-> \textit{Widening}~(\absexpr{\widen}) is crucial, computing an upper bound\\\textcolor{gray}{(We only need widening if the lattice has an infinite ascending chain!)}
\end{itemize}
...
\end{tikzpicture}
      \item<5-> Provide an integer concretization for~\absexpr{\gamma(\text{0})} and 
...
\begin{frame}[plain,c]
   \begin{tikzpicture}[overlay,remember picture]
      \node at(current page.center) {\LARGE\bfseries Bibliography};
   \end{tikzpicture}
\end{frame}
\AtBeginSection{}
...
\begin{frame}[plain,c]
   \begin{tikzpicture}[overlay,remember picture]
      \node at(current page.center) {\LARGE\bfseries A Little Bit of History};
   \end{tikzpicture}
\end{frame}
\def\Mixin{}%
\tikzset{%
