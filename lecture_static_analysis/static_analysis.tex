...
\ImageWithRoundedCorners{18.5mm}{edsger-dijkstra.jpg}%
}
...
\begin{frame}{Error Mining}
   ...
   \end{tikzpicture}
   \begin{itemize}
      \item<3->  We try to find code that has been \textit{almost} written like this!
      \item<4-> If many do something similar, but one does it differently, it might be a bug
      \item<5-> Usually relies on AST patterns with minimal binding information
   \end{itemize}\vfill
     
   \noindent\parbox{.8\linewidth}{\tiny\cite{10.1145/1052883.1052895}~\fullcite{10.1145/1052883.1052895}\\
   \cite{6224278}~\fullcite{6224278}}
\end{frame}
...
\end{tikzpicture}
\end{lrbox}
\begin{frame}[fragile]{A Best Friend for Life: Control Flow Graph (CFG) [TODO: CITES AND CROSS-REFS]}
...
\begin{tikzpicture}[overlay,remember picture]
   ...
   \onslide<11->{
      \node at(e-|@res) {r};
      \coordinate (mark@lr) at(e-|@res);
   }
   \onslide<12->{
      \node (bur2) at(end-|@res) {u};
      \node at(end-|@x) {u};
      \node (du2) at(end-|@y) {u};
   }
